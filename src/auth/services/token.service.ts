...
export class TokenService {
    ...
    createAccessToken(user: User): string {
        const expiresIn = `${this.configService.get<string>('ACCESS_TOKEN_EXP_TIME_IN_DAYS')}d`;
        return jwt.sign({ user }, this.configService.get<string>('ACCESS_TOKEN_SECRET'), { expiresIn });
    };

    createRefreshToken(refreshTokenEncode: RefreshTokneEncodeType): string {
        const expiresIn = `${this.configService.get<string>('REFRESH_TOKEN_EXP_TIME_IN_DAYS')}d`;
        return jwt.sign({ refreshTokenEncode }, this.configService.get<string>('REFRESH_TOKEN_SECRET'), { expiresIn });
    };
